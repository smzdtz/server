...
func (e EastMoney) QueryValuationStatus(ctx context.Context, secuCode string) (map[string]string, error) {
	...
	apiurl1, err := http.NewHTTPGetURLWithQueryString(ctx, apiurl, params)
	if err != nil {
		return nil, err
	}
	resp := RespValuation{}
	if err := http.HTTPGET(ctx, e.HTTPClient, apiurl1, nil, &resp); err != nil {
		return nil, err
	}
	...
	apiurl2, err := http.NewHTTPGetURLWithQueryString(ctx, apiurl, params)
	if err != nil {
		return nil, err
	}
	if err := http.HTTPGET(ctx, e.HTTPClient, apiurl2, nil, &resp); err != nil {
		return nil, err
	}
	...
	apiurl3, err := http.NewHTTPGetURLWithQueryString(ctx, apiurl, params)
	if err != nil {
		return nil, err
	}
	if err := http.HTTPGET(ctx, e.HTTPClient, apiurl3, nil, &resp); err != nil {
		return nil, err
	}
	...
	apiurl4, err := http.NewHTTPGetURLWithQueryString(ctx, apiurl, params)
	if err != nil {
		return nil, err
	}
	err = http.HTTPGET(ctx, e.HTTPClient, apiurl4, nil, &resp)
